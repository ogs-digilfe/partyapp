...
│   └── test_user.py
└── partyapp/
    ├── notebooks/                # Jupyterノートブック（分析・試作用）
    │   ├── 01_data_explore.ipynb
    │   └── 02_model_test.ipynb
    │
    ├── partyapp/                    # 実際のアプリケーションのPythonコード
    │   ├── __init__.py
    │   ├── config.py             # 設定ファイル（DB URIや環境変数読み込み）
    │   ├── db/                   # DB関連（SQLAlchemyモデルや接続設定）
    │   │   ├── __init__.py
    │   │   ├── base.py            # Baseクラス宣言 + セッション作成
    │   │   └── models/            # モデル定義
    │   │       ├── __init__.py
    │   │       └── user.py
    ...
    │   │   ├── __init__.py
    │   │   └── date_utils.py
    │   └── main.py                # エントリーポイント（後でFlaskやFastAPIに置き換え可）
    │
    ├── requirements.txt           # 必要ライブラリ
    ├── .env                        # ローカル環境変数（DB接続情報など）
    └── README.md
```
